...
public class TransactionDaoImpl extends connectDB implements TransactionDao {
    ...
    public void insert(Transactions transaction) {
        String sql = "INSERT INTO transactions(user_session,user_name,user_mail,user_phone,address,message,amount,payment,created,status,admin_id,department_id,confirm) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
        new connectDB();
        ...
        try {
            ...
            ps.setString(12, transaction.getDepartment_id());
            ps.setBoolean(13, transaction.isConfirm());
            ps.executeUpdate();
    ...
    public Transactions get(int id) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                transaction.setDepartment_id(rs.getString("department_id"));
    ...
    public void edit(Transactions transaction) {
        String sql = "Update transactions set user_name =?, user_mail =?, user_phone =?, address= ?,message=?,amount=?,payment=?, status=?, admin_id=?, department_id=?, confirm=? where id=?";
        ...
        try {
            ...
            ps.setString(10, transaction.getDepartment_id());
            ps.setBoolean(11, transaction.isConfirm());
    ...
    public int getTotalTransactions(String departmentId) {
        ...
        if (departmentId.equals("")) {
            ...
            sql = "SELECT COUNT(id) FROM transactions WHERE department_id = " + departmentId;
        }
        Connection conn = connectDB.getConnect();
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
    ...
    public List<Transactions> paging(int index, String departmentId) {
        ...
        String sql;
        if (departmentId.equals("")) {
            sql = "SELECT * FROM transactions ORDER BY id OFFSET ? ROWS FETCH NEXT 12 ROWS ONLY";
        } else {
            sql = "SELECT * FROM transactions WHERE department_id = "+departmentId+" ORDER BY id OFFSET ? ROWS FETCH NEXT 12 ROWS ONLY";
        }
        Connection conn = connectDB.getConnect();
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
